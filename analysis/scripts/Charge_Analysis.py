...
def main(fname,options):  
    ...
    print("Starting charge analysis")
    ##################################
    # Open data files
    ##################################    
    ...
    print(f"Loop on Events. ..................... Nevt={nevts}")
    ...
    for  ievt in  range(0,1000):
        ...
        print("Processing .... (event number, total number of events)")
        if TT() > 1.0:
            print(f"\r {ievt:10d}, {nevtot:10d}      ") 
            sys.stdout.flush()
            TT.reset()
    ...
    n, bins, patches = hist(C,X, density=True,color="mediumseagreen")
    mu, sigma = gaus.fit(C)
    y = gaus.pdf(bins, mu, sigma)
    l = plot(bins, y, 'r-', linewidth=2)
    p = Rectangle((0, 0), 1, 1, fc="r")
    legend(l, r'$\mu$=%.3f $\sigma$=%.1f' %( mu, sigma), loc='upper right')
    ...
    legend([p],[r'mean  %.1f sigma %.1f' % (fit_par[1], fit_par[2])], loc='upper right')
...
# Data histogram calibrated (the landau)
    ...
    y = dataFunc(fit_par, X)
    l = plot(X, y, 'r-', linewidth=2, label="hola")
    legend([p],[r'mode  %.1f sigma %.1f' % (fit_par[1], fit_par[2])],loc='upper right')
